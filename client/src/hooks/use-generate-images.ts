...
export const useGenerateImages = () => {
   ...
   */
  const generateImages = useCallback(async (wordText: string): Promise<string[] | null> => {
    ...
    try {
      ...
      const response = await axiosPrivate.post('/api/word/imagize', JSON.stringify({ slug: wordText }));
